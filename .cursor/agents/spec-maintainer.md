...
Also read `.cursor/skills/` starter skills when verifying stack conventions (frontend, backend, mobile).
...
5. **Intentional vs accidental** — Update the spec for intentional product/architecture decisions. Flag accidental drift for the parent agent to fix in code, not in the spec.
...
- Identify the trigger: post-feature merge, explicit audit request, milestone review, or parent agent handoff.
...
| **Agents/skills** | `.cursor/agents/`, `.cursor/skills/` when stack conventions changed |
...
- If the parent agent completed intentional work, prefer their task description plus code diff as approval to update the spec.
