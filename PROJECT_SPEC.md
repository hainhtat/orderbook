...
The product is **staff-only** in version one. There is no customer-facing portal, storefront, or self-service checkout. Shop owners and staff use a **web app** and **mobile app** in parallel against a shared versioned API.
...
| Client platforms (v1) | Web + mobile in parallel |
...
7. Deliver equivalent core workflows on web and mobile with Myanmar-first bilingual support.
...
- Web: responsive staff console (desktop-first; usable on tablet).
- Mobile: Expo app with parallel core flows (orders, customers, products, reports summary, AI chat).
- English/Myanmar toggle and light/dark theme on both.

**Technical**

- Versioned REST API at `/api/v1`.
- Prisma migrations (PostgreSQL canonical; SQLite for local dev and automated tests).
- Automated unit and integration tests on backend; baseline tests on web and mobile per starter skills.
...
| ORD-04 | Status flow: `DRAFT` → `CONFIRMED` → `COMPLETED` or `CANCELLED`. |
| ORD-05 | Completing order decrements stock (if not already reserved). |
...
| ID | Requirement |
| --- | --- |
| PAY-01 | Payment methods enum: `CASH`, `BANK_TRANSFER`, `KBZPAY_MANUAL`, `WAVE_MANUAL`, `OTHER` (manual recording only). |
...
| GET/PATCH | `/orders/:id` | Detail / update (limited fields) |
...
- **Notebook clarity**: high information density without clutter; strong typography and spacing (shadcn/ui web, Expo UI mobile).
...
### 9.4 Pre-order fulfillment lifecycle
...
### 9.5 AI order assistant workflow
...
### 9.6 Key empty and error states
...
- Scaffold `frontend/`, `backend/`, `mobile/` from starter skills.
- JWT auth, shop registration, tenant middleware.
- Health check, CI lint/typecheck/test on all three projects.

**Exit criteria:** Owner can register, create shop, log in on web and mobile.
...
**Exit criteria:** Owner manages catalog and customers on web and mobile.
...
- Pre-order pipeline views (web + mobile).
