import { ChevronRight, MapPin, PackageOpen, Plus, Search } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <Button asChild size="icon" className="h-12 w-12 shrink-0 rounded-full" aria-label={t('orders.addOrder')}>
          <Link to="/orders/new"><Plus className="h-6 w-6" /></Link>
        </Button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 [scrollbar-width:none] sm:mx-0 sm:px-0">
          {fulfillmentStatuses.map((value) => (
            ...
            >
              {value === 'ALL' ? t('orders.filters.all') : t(`orders.status.${value}`)}
            </Button>
          ))}
        </div>
        <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 [scrollbar-width:none] sm:mx-0 sm:px-0">
          {paymentStatuses.map((value) => (
            ...
            >
              {value === 'ALL' ? t('orders.paymentFilters.all') : t(`orders.paymentStatus.${value}`)}
            </Button>
          ))}
        </div>
      </div>
      ...
      {!isLoading && !isError && visibleOrders && visibleOrders.length > 0 ? (
        <div className="grid gap-3 lg:grid-cols-2">
          {visibleOrders.map((order) => {
            ...
            return (
            <article key={order.id} className="min-w-0 rounded-3xl border-l-4 border-l-rose-400 bg-card p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
